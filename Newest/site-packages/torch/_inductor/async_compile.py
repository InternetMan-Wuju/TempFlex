...
def _torch_npu_flex_kernel_kind(source_code: str, device_str: str):
    if device_str != "npu" or not isinstance(source_code, str):
        return None
    if "def bwd_dq_inner" in source_code or "THIS BLOCK DOES DK & DV" in source_code:
        return "backward"
    if "def forward_inner" in source_code and "KV_NUM_BLKS" in source_code and "FULL_KV_IDX" in source_code:
        return "forward"
    return None


def _torch_npu_wrap_flex_kernel(kernel, kind: str):
    if getattr(kernel, "_torch_npu_flex_debug_wrapped", False):
        return kernel
    run = getattr(kernel, "run", None)
    if run is None:
        return kernel

    def run_with_flex_debug(*args, **kwargs):
        run_count = getattr(kernel, "_torch_npu_flex_run_count", 0) + 1
        setattr(kernel, "_torch_npu_flex_run_count", run_count)
        print(
            f"[torch_npu flex_attention debug] 正在运行 torch_npu flex_attention {kind} kernel run#{run_count}",
            flush=True,
        )
        return run(*args, **kwargs)

    try:
        setattr(kernel, "run", run_with_flex_debug)
        setattr(kernel, "_torch_npu_flex_debug_wrapped", True)
        return kernel
    except Exception:
        class FlexKernelDebugProxy:
            _torch_npu_flex_debug_wrapped = True

            def __init__(self, inner):
                self._inner = inner
                self._torch_npu_flex_run_count = 0

            def __getattr__(self, name):
                return getattr(self._inner, name)

            def run(self, *args, **kwargs):
                self._torch_npu_flex_run_count += 1
                print(
                    f"[torch_npu flex_attention debug] 正在运行 torch_npu flex_attention {kind} kernel run#{self._torch_npu_flex_run_count}",
                    flush=True,
                )
                return run(*args, **kwargs)

        return FlexKernelDebugProxy(kernel)


def _torch_npu_wrap_flex_triton_result(result, kind):
    if kind is None:
        return result
    if isinstance(result, CodeCacheFuture):
        original_result = result.result

        def wrapped_result():
            return _torch_npu_wrap_flex_kernel(original_result(), kind)

        return LambdaFuture(wrapped_result, future=getattr(result, "future", None))
    return _torch_npu_wrap_flex_kernel(result, kind)
...
class AsyncCompile:
    ...
    def triton(self, kernel_name: str, source_code: str, device_str: str = "cuda"):
        ...
        """
        flex_debug_kind = _torch_npu_flex_kernel_kind(source_code, device_str)

        if future := CompiledTritonKernels.get(source_code, None):
            counters["inductor"]["async_compile_cache_hit"] += 1
            return _torch_npu_wrap_flex_triton_result(future, flex_debug_kind)
        ...
        if os.environ.get("TRITON_INTERPRET", "0") == "1":
            return _torch_npu_wrap_flex_triton_result(getattr(
        ...
        if is_parallel:
            ...
            with dynamo_timed(
                ...
                return _torch_npu_wrap_flex_triton_result(kernel, flex_debug_kind)
