...
def _can_use_causal_sdpa_fastpath(
    query,
    key,
    value,
    subgraph,
    ...
    if query.get_device() != key.get_device() or query.get_device() != value.get_device():
        return False
    # Keep this fastpath narrow: F.scaled_dot_product_attention's causal semantics
    # are an exact match for token_q >= token_kv in this square, non-GQA case.
    return (
        q_size[0] == k_size[0] == v_size[0]
        and q_size[1] == k_size[1] == v_size[1]
        and q_size[2] == k_size[2] == v_size[2]
        and q_size[3] == k_size[3]
        and k_size[3] == v_size[3]
    )


def _lower_causal_sdpa_fastpath(query, key, value, scale):
    query, key, value = maybe_realize([query, key, value])
    ...
    B, H, seq_len_q, _ = query.get_size()
...
def _register_npu_inductor_flex_attention():
    ...
    def flex_attention(
        ...
        # ── 优化：禁用 SDPA fastpath，强制走 flex_attention Triton kernel ──
        # if _can_use_causal_sdpa_fastpath(
        #     query, key, value, subgraph, mask_graph,
        #     kernel_options, score_mod_other_buffers, mask_mod_other_buffers,
        # ):
        #     _debug_print("使用 torch_npu causal SDPA fastpath 替代通用 flex_attention Triton kernel")
        #     return _lower_causal_sdpa_fastpath(query, key, value, scale)
        ...
        if use_inline_causal:
            _debug_print("检测到 identity+causal，启用 INLINE_CAUSAL_MASK + PRESCALE_QK 优化")
            kernel_options.setdefault("INLINE_CAUSAL_MASK", True)
            kernel_options.setdefault("PRESCALE_QK", True)
