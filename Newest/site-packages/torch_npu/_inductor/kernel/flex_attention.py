...
{{def_kernel("Q", "K", "V", "LSE", "KV_NUM_BLKS", "KV_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX", "PERM")}}
    ...
    # PERM: Reorder permutation (1D int32, size=n_mask_blocks).
    #        PERM[i] = original mask Q-block to process at reordered position i.
    #        When identity (PERM[i]=i), behaviour matches the original kernel.
    #
    ...
    # PERM is at mask-block level (size = n_mask_blocks).
    # When ENABLE_REORDER=True, PERM[i] = original mask Q-block at reordered position i.
    # Expand to triton-block level: multiple triton blocks share the same mask block.
    if ENABLE_REORDER:
    ...
    sparse_kv_num_blks_offset = sparse_hz_offset * stride_kv_num_blks_h + src_block // SPARSE_Q_MULTIPLE
    sparse_kv_idx_offset = sparse_hz_offset * stride_kv_idx_h + (src_block // SPARSE_Q_MULTIPLE) * stride_kv_idx_m  # noqa: B950

    Q_block_ptr = tl.make_block_ptr(
        base=Q,
        shape=(Q_LEN, QK_HEAD_DIM),
        strides=(stride_qm, stride_qk),
        offsets=(src_block * BLOCK_M, 0),
...
{{def_kernel("Q", "K", "V", "LSE", "KV_NUM_BLKS", "KV_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX", "PERM")}}
    ...
    # Wave-based processing: each program handles WAVE_SIZE Q blocks.
    # The grid is reduced by WAVE_SIZE (see _flex_attention_grid_with_wave).
    wave_start = wave_id * WAVE_SIZE
    for wave_w in range(WAVE_SIZE):
        q_start = wave_start + wave_w
        # PERM lookup for this Q block in the wave
        if ENABLE_REORDER:
            src_mask_block = tl.load(PERM + (q_start // SPARSE_Q_MULTIPLE))
            src_block = src_mask_block * SPARSE_Q_MULTIPLE + (q_start % SPARSE_Q_MULTIPLE)
        else:
...
# ============================================================
# Side-channel for passing PERM tensor from eager code to lowering
# torch.compile tracing makes direct tensor passing through
# kernel_options impossible (meta-tensor issue). Use this
# module-level variable as a workaround.
# ============================================================
_PENDING_PERM = None


def set_pending_perm(perm: "torch.Tensor"):
    """Set the reorder permutation for the next flex_attention call.

    Args:
        perm: 1D int32 CPU tensor of size n_mask_blocks.
              perm[i] = original mask Q-block at reordered position i.
    """
    global _PENDING_PERM
    _PENDING_PERM = perm


def _get_and_clear_perm():
    """Get and clear the pending permutation. Returns None if not set."""
    global _PENDING_PERM
    perm = _PENDING_PERM
    _PENDING_PERM = None
    return perm
...
flex_attention_backward_template = TritonTemplate(
    ...
    source=r"""
{{def_kernel("Q", "K", "V", "LSE", "DELTA", "DO", "DQ", "DV", "KV_NUM_BLKS", "KV_IDX", "Q_NUM_BLKS", "Q_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX", "FULL_Q_NUM_BLKS", "FULL_Q_IDX", "PERM")}}
    # Sub notation for this kernel:
    #
    # Q: Query, K: Key, V: Value
...
def _register_npu_inductor_flex_attention():
    ...
    def flex_attention(
        ...
        # WAVE_SIZE: number of Q blocks per program (default 1 = original behavior)
        kernel_options.setdefault("WAVE_SIZE", 1)
        # PERM: 1D int32 tensor (mask-level Q-block permutation).
        # Use side-channel to receive perm from eager code (via set_pending_perm).
        pending_perm = _get_and_clear_perm()
        if pending_perm is not None:
            kernel_options.setdefault("ENABLE_REORDER", True)
            # Convert concrete tensor to inductor constant buffer
            from torch._inductor.graph import GraphLowering
            perm = V.graph.add_tensor_constant(pending_perm)
        else:
            kernel_options.setdefault("ENABLE_REORDER", False)
            # Dummy PERM (size 1) — kernel won't access it when ENABLE_REORDER=False
            perm = empty(1, dtype=torch.int32, device=query.get_device())
            perm = maybe_realize([perm])[0]
        ...
        for BLOCK_M, BLOCK_N, num_warps, num_stages in configs:
            ...
            # Blocksparse options
            cur_kernel_options.setdefault("WAVE_SIZE", kernel_options.get("WAVE_SIZE", 1))
        ...
        input_gen_fns = {
            ...
            8: create_num_blocks_fake_generator(perm),
        }
        return (
    ...
    def flex_attention_backward(*args, **kwargs):
        ...
        # PERM: identity by default. Backward doesn't support reorder yet.
        kernel_options.setdefault("ENABLE_REORDER", False)
        enable_reorder_bwd = kernel_options.get("ENABLE_REORDER", False)
        perm_tensor_bwd = kernel_options.pop("PERM", None)
        if perm_tensor_bwd is not None and enable_reorder_bwd:
            perm_bwd = maybe_realize([perm_tensor_bwd])[0]
        else:
            perm_bwd = empty(1, dtype=torch.int32, device=query.get_device())
            perm_bwd = maybe_realize([perm_bwd])[0]
        ...
        input_gen_fns = {
            ...
            16: create_num_blocks_fake_generator(perm_bwd),
        }
