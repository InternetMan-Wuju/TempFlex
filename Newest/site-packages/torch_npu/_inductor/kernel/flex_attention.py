...
def _flex_attention_kernel_kind(src):
    if not isinstance(src, str):
        return None
    # backward: dq/dk/dv 特征
    if "def bwd_dq_inner" in src or "THIS BLOCK DOES DK & DV" in src:
        return "backward"
    # forward: 不靠函数名，靠参数里的 KV_NUM_BLKS + FULL_KV_IDX 特征
    if ("arg_KV_NUM_BLKS" in src or "KV_NUM_BLKS" in src) and \
       ("arg_FULL_KV_IDX" in src or "FULL_KV_IDX" in src) and \
       "BLOCK_M" in src:
        return "forward"
    return None


def _flex_attention_static_int(src, name):
    if not isinstance(src, str):
        return None
    match = re.search(
        rf"^\s*{re.escape(name)}\s*(?::\s*tl\.constexpr)?\s*=\s*([0-9]+)\b",
        src,
        re.MULTILINE,
    )
    if match is None:
...
def _flex_attention_signature_dtype(autotuner):
    signature = getattr(autotuner, "triton_meta", {}).get("signature", {})
    values = []
    if isinstance(signature, dict):
        for key in ("Q", 0):
            if key in signature:
                values.append(signature[key])
        values.extend(signature.values())
    for value in values:
...
def _flex_attention_device(autotuner):
    device_props = getattr(autotuner, "device_props", None)
    device_type = getattr(device_props, "type", None)
    device_index = getattr(device_props, "index", None)
    if device_type is None:
        triton_meta = getattr(autotuner, "triton_meta", {})
        device_type = triton_meta.get("device_type", "npu")
        device_index = triton_meta.get("device", device_index)
    if device_index is None:
        return str(device_type)
    return f"{device_type}:{device_index}"


def _flex_attention_compile_message(autotuner, kind):
    src = getattr(getattr(autotuner, "fn", None), "src", None)
    shape = [
        _flex_attention_static_int(src, "ZQ"),
        _flex_attention_static_int(src, "HQ"),
        _flex_attention_static_int(src, "Q_LEN"),
        _flex_attention_static_int(src, "QK_HEAD_DIM"),
    ]

    message = f"正在编译 torch_npu flex_attention {kind} kernel"
    if all(dim is not None for dim in shape):
        message += f" shape={shape}"

    dtype = _flex_attention_signature_dtype(autotuner)
    if dtype is not None:
        message += f" dtype={dtype}"

    device = _flex_attention_device(autotuner)
    if device is not None:
        message += f" device={device}"
    return message


def _flex_attention_method_has_run_debug(method):
    ...
    return (
        "_torch_npu_flex_run_count" in names
        or any("正在运行 torch_npu flex_attention" in value for value in constants)
    )


def _install_runtime_debug_hook():
    try:
        from torch._inductor.runtime.triton_heuristics import CachingAutotuner
    except Exception:
        return

    def patch_autotuner_class(autotuner_cls):
        if autotuner_cls.__dict__.get("_torch_npu_flex_debug_hooked", False):
            return

        original_precompile_worker = autotuner_cls._precompile_worker

        def precompile_worker_with_flex_debug(self):
            kind = _flex_attention_kernel_kind(getattr(getattr(self, "fn", None), "src", None))
            if (
                kind is not None
                and not getattr(self, "compile_results", None)
                and not getattr(self, "launchers", None)
            ):
                _debug_print(_flex_attention_compile_message(self, kind))
            return original_precompile_worker(self)

        autotuner_cls._precompile_worker = precompile_worker_with_flex_debug

        original_run = getattr(autotuner_cls, "run", None)
        if original_run is not None and not _flex_attention_method_has_run_debug(original_run):
            @functools.wraps(original_run)
            def run_with_flex_debug(self, *args, **kwargs):
                kind = _flex_attention_kernel_kind(getattr(getattr(self, "fn", None), "src", None))
                if kind is not None and not getattr(self, "_torch_npu_flex_debug_wrapped", False):
                    run_count = getattr(self, "_torch_npu_flex_run_count", 0) + 1
                    self._torch_npu_flex_run_count = run_count
                    _debug_print(
                        f"正在运行 torch_npu flex_attention {kind} kernel run#{run_count}"
                    )
                return original_run(self, *args, **kwargs)

            autotuner_cls.run = run_with_flex_debug

        autotuner_cls._torch_npu_flex_debug_hooked = True

    patch_autotuner_class(CachingAutotuner)
    try:
        from torch_npu._inductor.npu_triton_heuristics import NPUCachingAutotuner
    except Exception:
        return
    patch_autotuner_class(NPUCachingAutotuner)


_install_runtime_debug_hook()
...
def forward_block_mn(
    ...
    {% else %}
    # 内联优化：identity score_mod，不做任何变换
    post_mod_scores = qk
    {% endif %}
    ...
    if not IS_FULL_BLOCKS:
        ...
        {% else %}
        # 内联优化：causal mask，直接比较 token 位置
...
{{def_kernel("Q", "K", "V")}}
    # Specialized identity-score + causal-mask Flex Attention forward kernel.
    # It keeps the Flex Triton implementation path, but removes block-sparse
    # metadata traffic and the partial/full split used by the generic template.
    ...
    for start_n in range(0, full_block_end):
        acc, l_i, m_i = causal_forward_block_mn(
            q, K_block_ptr, V_block_ptr, Q_LEN, KV_LEN,
            acc, l_i, m_i,
            offs_m[:, None], offs_n[None, :],
            MATMUL_PRECISION, RCP_LN2,
            IS_CAUSAL_TAIL=False,
...
@triton.jit
def causal_forward_block_mn(
    q, K_block_ptr, V_block_ptr, Q_LEN, KV_LEN,
    acc, l_i, m_i,
    offs_m, offs_n,
    MATMUL_PRECISION, RCP_LN2,
    IS_CAUSAL_TAIL: tl.constexpr,
...
causal_flex_attention_template = TritonTemplate(
    ...
    grid=flex_attention_grid,
    source=compute_causal_flex_attention
    + compute_causal_forward_block_mn
    + load_checked_block,
)
...
def _register_npu_inductor_flex_attention():
    ...
    def flex_attention(
        ...
        # ── 优化：检测 identity score_mod + causal mask_mod，启用内联优化 ──
        use_inline_causal = (
            _is_identity_score_graph(subgraph)
            and _is_causal_mask_graph(mask_graph)
            and len(score_mod_other_buffers) == 0
            and len(mask_mod_other_buffers) == 0
        )
        if use_inline_causal:
            _debug_print("检测到 identity+causal，启用 Flex causal 优化候选")
            kernel_options.setdefault("INLINE_CAUSAL_MASK", True)
            # 创建空的 subgraph buffer（不会被用到，但框架需要传递）
            fake_score_placeholders = [
            ...
            subgraph_buffer = build_subgraph_buffer(fake_score_placeholders, subgraph)
            fake_mask_placeholders = [
                create_placeholder(name, dtype, query.get_device())
                for name, dtype in [
                    ("b", torch.int32),
                    ("h", torch.int32),
            ...
            mask_graph_buffer = build_subgraph_buffer(fake_mask_placeholders, mask_graph)
        else:
            placeholder_inps = [
                create_placeholder(name, dtype, query.get_device())
                for name, dtype in [
        ...
        if _can_use_causal_flex_template(
            ...
            cur_kernel_options.setdefault("num_stages", causal_config[3])
            cur_kernel_options.setdefault("num_warps", causal_config[2])
            cur_kernel_options.setdefault("BLOCK_M", causal_config[0])
            cur_kernel_options.setdefault("BLOCK_N", causal_config[1])

            if cur_kernel_options["BLOCK_M"] <= 64 and cur_kernel_options["BLOCK_N"] <= 64:
                choices: list[Any] = []
                error = causal_flex_attention_template.maybe_append_choice(
                    choices=choices,
                    input_nodes=[
                        query,
                        key,
                ...
                if error is not None:
                    _debug_print(f"dense causal Flex template 不可用，回退通用模板: {error}")
