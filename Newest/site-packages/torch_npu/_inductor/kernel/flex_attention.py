...
# PERM-based variant: same as flex_attention but q_start reads from PERM pointer
_comp_perm = compute_flex_attention.replace(
    '{{def_kernel("Q", "K", "V", "LSE", "KV_NUM_BLKS", "KV_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX")}}',
    '{{def_kernel("Q", "K", "V", "LSE", "KV_NUM_BLKS", "KV_IDX", "FULL_KV_NUM_BLKS", "FULL_KV_IDX", "PERM")}}'
).replace(
    "    q_start = tl.program_id(0)",
...
def _register_npu_inductor_flex_attention():
    ...
    def flex_attention(
        ...
        perm_ir_val = None
        if pending_perm is not None:
            perm_tensor_c = pending_perm.to(torch.int32).to(query.get_device())
            perm_ir_val = V.graph.add_tensor_constant(perm_tensor_c, name="perm_reorder")
        ...
        if _can_use_causal_flex_template(
            ...
            if cur_k["BLOCK_M"] <= 64 and cur_k["BLOCK_N"] <= 64:
                if perm_ir_val is not None:
                    # Causal + PERM: use causal-perm dense template
                    choices_causal_perm = []
                    err = causal_flex_attention_perm_template.maybe_append_choice(
                        choices=choices_causal_perm,
                        input_nodes=[query, key, value, perm_ir_val],
                        layout=layout, subgraphs=[], mutated_inputs=[],
                        call_sizes=query.get_size(), **cur_k,
                    )
                    if err is None and choices_causal_perm:
                        _debug_print("使用 torch_npu dense causal+PERM Flex Triton template")
                        return (
                            autotune_select_algorithm(
                                "flex_attention_causal_perm", choices_causal_perm,
                                [query, key, value, perm_ir_val], layout,
                            ),
                            logsumexp,
                        )
                    if err is not None:
                        _debug_print(f"causal+PERM template 不可用: {err}")
                else:
        ...
        # ── Reorder: perm-based generic template (non-causal with perm) ──
        if perm_ir_val is not None:
