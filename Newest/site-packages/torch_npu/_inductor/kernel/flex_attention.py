...
def _install_runtime_debug_hook():
    ...
    def patch_autotuner_class(autotuner_cls):
        ...
        original_make_launchers = autotuner_cls._make_launchers

        def make_launchers_with_flex_debug(self):
            result = original_make_launchers(self)
            ...
            for launcher in self.launchers:
                if getattr(launcher, "_torch_npu_flex_debug_wrapped", False):
                    wrapped_launchers.append(launcher)
                    continue

                @functools.wraps(launcher)
                def debug_launcher(*args, _launcher=launcher, _kind=kind, **kwargs):
                    _debug_print(f"正在运行 torch_npu flex_attention {_kind} kernel")
                    return _launcher(*args, **kwargs)

                debug_launcher.__dict__.update(getattr(launcher, "__dict__", {}))
                debug_launcher._torch_npu_flex_debug_wrapped = True
                wrapped_launchers.append(debug_launcher)
            self.launchers = wrapped_launchers
            return result

        autotuner_cls._make_launchers = make_launchers_with_flex_debug
...
def _register_npu_inductor_flex_attention():
    ...
    def flex_attention(
        ...
        _debug_print(
            f"正在编译 torch_npu flex_attention forward kernel "
            f"shape={query.get_size()} dtype={query.get_dtype()} device={query.get_device()}"
        )
        # below is npu path
        (
            _,  # q_length
    ...
    def flex_attention_backward(*args, **kwargs):
        ...
        _debug_print(
            f"正在编译 torch_npu flex_attention backward kernel "
            f"shape={query.get_size()} dtype={query.get_dtype()} device={query.get_device()}"
        )
        (
            _,  # q_length
