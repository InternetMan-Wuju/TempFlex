...
{{def_kernel("Q", "K", "V")}}
    ...
    for start_n in range(0, full_block_end):
        acc, l_i, m_i = causal_forward_block_mn(q, K_block_ptr, V_block_ptr, Q_LEN, KV_LEN, acc, l_i, m_i, offs_m[:, None], offs_n[None, :], MATMUL_PRECISION, RCP_LN2, IS_CAUSAL_TAIL=False, CHECK_BLOCK_BOUNDARY=False)
        K_block_ptr = tl.advance(K_block_ptr, (0, BLOCK_N))
        V_block_ptr = tl.advance(V_block_ptr, (BLOCK_N, 0))
        offs_n = offs_n + BLOCK_N
    for start_n in range(full_block_end, partial_block_end):
        acc, l_i, m_i = causal_forward_block_mn(q, K_block_ptr, V_block_ptr, Q_LEN, KV_LEN, acc, l_i, m_i, offs_m[:, None], offs_n[None, :], MATMUL_PRECISION, RCP_LN2, IS_CAUSAL_TAIL=True, CHECK_BLOCK_BOUNDARY=not IS_DIVISIBLE)
        K_block_ptr = tl.advance(K_block_ptr, (0, BLOCK_N))
        V_block_ptr = tl.advance(V_block_ptr, (BLOCK_N, 0))
        offs_n = offs_n + BLOCK_N
    acc = acc / l_i[:, None]
...
def _register_npu_inductor_flex_attention():
    ...
    def flex_attention(
        ...
        # ── Causal fastpath ──
        # NOTE: When the causal fastpath is eligible, we always use the standard
        # causal template (no PERM). Causal dense attention already has optimal KV
        # cache access patterns (sequential) — Q-block reorder doesn't help here.
        # PERM is only used for the generic block-sparse template below.
