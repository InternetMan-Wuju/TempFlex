...
def _torch_npu_flex_kernel_kind(src):
...
def _torch_npu_flex_debug_print(message):
...
class NPUCachingAutotuner(CachingAutotuner):
    ...
    def run(
        ...
        flex_debug_kind = _torch_npu_flex_kernel_kind(getattr(getattr(self, "fn", None), "src", None))
        if flex_debug_kind is not None:
            run_count = getattr(self, "_torch_npu_flex_run_count", 0) + 1
            self._torch_npu_flex_run_count = run_count
            _torch_npu_flex_debug_print(
                f"正在运行 torch_npu flex_attention {flex_debug_kind} kernel run#{run_count}"
            )
